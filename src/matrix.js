...
function update_matrix(callback) {
	/*d3.select('#matrixg').selectAll(".cell")
		  ...
		  .style("fill", function(d) { 
			return ((mygraph.nodes[d.x].group ==2) || (mygraph.nodes[d.y].group==2)) ? c(2) : null; 
		  });  */
	
	d3.select("#matrixg").selectAll(".rowr")
		.transition()
		.style("fill", function(d, i) { 
			return mygraph.nodes[i].group == 2 ? c(mygraph.nodes[i].group) : "white"; 
		  })
		  .style("fill-opacity", function(d, i) { 
			return mygraph.nodes[i].group == 2 ? 0.8 : 0; 
		  });
...
function draw_matrix(svg, mygid, callback) {
...
function color_mx_groups(callback){
	var g1, g2;
	d3.select('#matrixg').selectAll('.cell')
		.transition()
		.style("fill", function(d){
			mygraph.nodes.forEach(function(node, idx){
				if (d.x == idx) g1 = node.group;
				if (d.y == idx) g2 = node.group;
			});
			return c(Math.min(g1,g2));
